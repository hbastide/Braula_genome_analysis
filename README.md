...
<br>Note: The following was changed in the _Braula_ analysis as compared to the default configuration file:
```bash
...
<br>Note: Braula_masurca403.fasta.PolcaCorrected.fa was then renamed Braula_assembly_21_09_12.fasta
```bash
...
## Genome annotation
Note: Commands are given for _Braula_ but the same commands were conducted to annotate _Leucophenga varia_, _Phortica variegata_ and _Ephydra gracilis_
...
```
##Two libraries were used to annotate repeats : the "all" library of Repbase ($model_org) and the library specific to Braula coeca generated by RepeatModeler ($rmlib)
##A fasta file of transposable elements proteins was also provided to Maker : RepeatPeps.lib ($repeat_protein)
##option softmask=1
##5 proteomes of related organisms were provided to Maker ($protein) : Drosophila innubila, D. albomicans, D. bipectinata, D. melanogaster and D. virilis ($protein2genome=1)
...
9333 genes, 9333 OK, 4386 warnings, 0 errors
...
##2nd round of gene prediction with Maker
